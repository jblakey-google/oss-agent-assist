...
describe("c-agent-assist-container-module", () => {
  ...
  describe("wiredVoiceCall", () => {
    let mockPlatformService;

    beforeEach(() => {
      mockPlatformService = {
        handleSessionIdUpdated: jest.fn(),
        getSessionId: jest.fn(
          (data) => data?.fields?.Five9BYOT__F9_SessionId__c?.value || null
        )
      };
      element.platformService = mockPlatformService;
    });

    it("extracts session_id reactively and triggers platformService callback", async () => {
      const wiredVoiceCallCallback =
        AgentAssistContainerModule.prototype.wiredVoiceCall;
      expect(wiredVoiceCallCallback).toBeDefined();

      const mockData = {
        fields: {
          VendorCallKey: { value: "test-vendor-call-key" },
          Five9BYOT__F9_SessionId__c: { value: "test-five9-session-id" }
        }
      };

      wiredVoiceCallCallback.call(element, { data: mockData });

      expect(element.voiceCallData).toEqual(mockData);
      expect(element.vendorCallKey).toBe("test-vendor-call-key");
      expect(element.sessionId).toBe("test-five9-session-id");
      expect(mockPlatformService.handleSessionIdUpdated).toHaveBeenCalledWith(
        "test-five9-session-id"
      );
    });
  });
});
