...
export default class AgentAssistContainerModule extends LightningElement {
  ...
  // @api endpoint; // Configured in App Builder: e.g. https://your-ui-connector-endpoint.a.run.app (defined below under LWC Getters)
  ...
  wiredVoiceCall({ error, data }) {
    if (data) {
      ...
      this.debugLog(
        `Wired VoiceCall record updated! Data: ${JSON.stringify(data)}`
      );
  ...
  // LWC Getters - deconstruct values from @wire and @api objects on demand
  _endpoint = "";
  @api
  get endpoint() {
  ...
  set endpoint(value) {
    if (!value) return;
    this._endpoint = value.endsWith("/") ? value.slice(0, -1) : value;
  }
