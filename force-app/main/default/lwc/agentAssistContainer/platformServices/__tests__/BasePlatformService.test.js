...
describe("BasePlatformService", () => {
  ...
  describe("initUIModules", () => {
    ...
    it("prevents duplicate initialization and logs warning", () => {
      basePlatformService.initUIModules(); // First init

      // Clear call mock history
      global.document.createElement.mockClear();
      mockLwc.refs.agentAssistContainer.appendChild.mockClear();

      basePlatformService.initUIModules(); // Second duplicate init

      expect(global.document.createElement).not.toHaveBeenCalled();
      expect(
        mockLwc.refs.agentAssistContainer.appendChild
    ...
    it("clears DOM container elements before appending new instances", () => {
      basePlatformService.initUIModules();
      expect(mockLwc.refs.agentAssistContainer.innerHTML).toBe("");
      expect(mockLwc.refs.agentAssistTranscript.innerHTML).toBe("");
    });
  });
  ...
  describe("pollForConversationNameByIntegrationKey", () => {
    ...
    it("polls periodically and backs off", async () => {
      basePlatformService.pollForConversationNameByIntegrationKey("test-key", {
        initialDelay: 1000,
        maxDelay: 10000,
        requestTimeoutMs: 9900
      });

      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(1);

      // Advance timers by 1000ms (initial delay) to trigger second poll
      await jest.advanceTimersByTimeAsync(1000);
      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(2);

      // Next delay will be delay + (maxDelay - initialDelay) / 10 = 1000 + 900 = 1900ms
      await jest.advanceTimersByTimeAsync(1900);
      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(3);
    });

    it("stops polling on success", async () => {
      fetchConversationNameSpy.mockResolvedValue("found-conversation-name");

      basePlatformService.pollForConversationNameByIntegrationKey("test-key");

      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(1);

      // Let microtasks run so that the success logic propagates
      await Promise.resolve();

      // Ensure no further timeouts are scheduled
      await jest.runAllTimersAsync();
      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(1);
    });

    it("aborts active requests and stops polling on teardown", async () => {
      basePlatformService.pollForConversationNameByIntegrationKey("test-key");

      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(1);

      const abortSpy = jest.spyOn(basePlatformService.pollController, "abort");

      basePlatformService.teardown();

      expect(abortSpy).toHaveBeenCalled();
      expect(basePlatformService.pollingTimeout).toBeFalsy();

      // Fast-forward all timers to check if any new polls got scheduled
      await jest.runAllTimersAsync();
      // Should still be 1 (first initial call before teardown)
      expect(fetchConversationNameSpy).toHaveBeenCalledTimes(1);
    });
  });
});
